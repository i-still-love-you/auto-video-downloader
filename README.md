...
| 영역 | 내용 |
| --- | --- |
| 동영상 자동 감지 | 내장 브라우저(WebContentsView)의 네트워크 응답을 감시해 m3u8(HLS), mpd(DASH), mp4/webm/mkv/mov/flv 등 직접 파일을 찾아냅니다. Referer / Cookie / User-Agent 를 함께 저장해 다운로드 시 그대로 전달합니다. 감지 목록은 브라우저 오른쪽에 항상 고정된 사이드바로 표시되며(즐겨찾기·기록·광고 차단 패널은 그 위에 잠깐 펼쳐짐), 감지 항목에는 이미 받은 파일(다운로드됨)과 진행 중인 항목이 배지로 표시되고, "바로 받기" 버튼은 분석 창 없이 기본 화질 설정으로 즉시 큐에 넣습니다. |
